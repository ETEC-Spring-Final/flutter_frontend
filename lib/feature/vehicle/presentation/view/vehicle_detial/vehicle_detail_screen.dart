...
class _VehicleDetailScreenState extends State<VehicleDetailScreen> {
  ...
  /// Days the vehicle is already reserved (unavailable for rent), loaded from
  /// the Spring Boot `booked-dates` endpoint.
  ...
  Future<List<BookedDate>> _loadBookedDates() async {
    // Reuse the same repository instance that backs VehicleBloc. Failures are
    // treated as "no data" so the screen never blocks on a network error.
    final result = await context
        .read<VehicleBloc>()
        .repository
        .getVehicleBookedDates(widget.vehicle.id);

    return result.fold((failure) => const <BookedDate>[], (dates) => dates);
  }
  ...
  Widget _buildUnavailableDays(BuildContext context) {
    final theme = Theme.of(context);
